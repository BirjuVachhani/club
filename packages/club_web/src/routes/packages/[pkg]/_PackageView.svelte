<script lang="ts">
  ...
  async function copyReadme() {
    ...
    try {
      ...
      readmeCopied = true;
      setTimeout(() => (readmeCopied = false), 1500);
    } catch {
          ...
          {#if activeTab === "readme"}
            {#if pkg.readme}
              <div class="readme-tab">
                <button
                  class="readme-copy-btn"
                  type="button"
                  onclick={copyReadme}
                  title={readmeCopied ? "Copied" : "Copy readme"}
                  aria-label={readmeCopied ? "Readme copied" : "Copy readme"}
                >
                  {#if readmeCopied}
                    <svg
                      width="14"
            ...
            {:else}
              <p class="empty-tab">No readme available.</p>
            {/if}
...
<style>
  ...
  .readme-copy-btn:hover {
    color: var(--foreground);
    border-color: var(--pub-muted-text-color);
  }
  .readme-copy-btn:focus-visible {
