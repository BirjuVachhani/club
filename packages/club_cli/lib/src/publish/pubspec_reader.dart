...
PackagePubspec readPubspec(String packageDir, {String? versionOverride}) {
  ...
  final Pubspec parsed;
  try {
    parsed = Pubspec.parse(raw, sourceUrl: pubspecFile.uri);
  } on Exception catch (e) {
    throw FormatException('Invalid pubspec.yaml: $e');
  }

  // Convert the YAML node to a plain Dart map so downstream code does not
  // have to know about YamlMap/YamlList.
  final yamlNode = loadYaml(raw, sourceUrl: pubspecFile.uri);
