...
void main() {
  ...
  group('running from source', () {
    ...
    test('refuses even with --force', () async {
      final code =
          await run(UpgradeOptions(force: true), latest: release('9.9.9'));
      expect(code, ExitCodes.config);
    }, skip: runningFromSource ? null : 'only meaningful under dart run');
  });

  group('asset availability gate', () {
    test('a half-published release reports as up to date, not available',
        () async {
    ...
    test('a release missing only SHA256SUMS.txt also reports up to date',
        () async {
      final code = await run(
        UpgradeOptions(check: true),
        latest: release('9.9.9', assets: ['club-cli-9.9.9-macos-arm64.tar.gz']),
      );
      expect(code, ExitCodes.success);
    });

    test('an explicit --version for a pending release is a hard error',
        () async {
      final code = await UpgradeRunner(
  ...
  group('writability precheck', () {
    test('a read-only destination fails before downloading anything',
        () async {
      ...
      expect(code, ExitCodes.config);
        ...
        skip: Platform.isWindows
            ? 'chmod semantics differ on Windows'
            : null);
  });
}
