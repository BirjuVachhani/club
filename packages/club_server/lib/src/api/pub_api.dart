...
class PubApi {
  ...
  DecodedRouter get router {
    ...
    // Screenshot asset proxy. Auth-required (Club is private). Same-origin
    // <img src> on the package detail page carries the session cookie, so
    // browser rendering still works.
    router.get(
    ...
    // README links to with relative paths). Auth-required for the same
    // reason as screenshots — the rewritten README in `readme_content`
    // points here and same-origin requests carry the session cookie.
    router.get(
