      ...
      .addMiddleware(
        authMiddleware(
          ...
          publicPaths: {
            '/api/v1/health',
            '/api/auth/login',
            '/api/auth/me', // returns 401 on its own if unauthenticated
            '/api/auth/signup',
            '/api/setup/',
            '/api/invites/',
            '/api/packages',
            '/api/archives/',
            '/api/search',
            '/api/legal/', // public privacy/terms reads; admin mutations live under /api/admin/legal
            '/api/users/', // only /api/users/<id>/avatar is registered; narrow if more routes added
            '/documentation/',
            '/oauth/authorize',
            '/oauth/token',
          },
