// Applies club theme to a dartdoc-generated static site.
//
// Strategy (intentionally minimal so dartdoc's search/theme-toggle JS keeps
// working unchanged):
//
//   1. Mirror work/<pkg>/doc/api → output/themed/
//   2. Drop a `club-theme.css` and `club-theme.js` into static-assets/
//      that override dartdoc's CSS custom properties (--main-*) with club's
//      palette, swap fonts, and round/restyle key surfaces.
//   3. Walk every .html and inject a stylesheet/script <link> just before
//      </head>, with the correct relative path back to static-assets/.
//
// Usage: dart run bin/theme.dart [path/to/baseline/api]
//   Default baseline: first work/*/doc/api detected.
...
Future<void> main(List<String> args) async {
  ...
  final baseline = args.isNotEmpty
      ? (p.isAbsolute(args[0]) ? args[0] : p.normalize(p.join(root, args[0])))
      : _detectBaseline(root);

  if (!Directory(baseline).existsSync()) {
    stderr.writeln('Baseline not found: $baseline');
    stderr.writeln('Run `dart run bin/setup.dart` first.');
    exit(1);
  }

  final output = Directory(p.join(root, 'output', 'themed'));
  if (output.existsSync()) output.deleteSync(recursive: true);
  output.createSync(recursive: true);

  stdout.writeln('Mirroring $baseline → ${output.path}');
  _copyTree(Directory(baseline), output);

  // Drop our overrides into static-assets/.
  final staticAssets = Directory(p.join(output.path, 'static-assets'));
  staticAssets.createSync(recursive: true);
  File(p.join(staticAssets.path, 'club-theme.css'))
      .writeAsStringSync(_clubThemeCss);
  File(p.join(staticAssets.path, 'club-theme.js'))
      .writeAsStringSync(_clubThemeJs);

  // Copy the club logo into static-assets/ so we can use it as the
  // favicon for the docs site (matches club_web).
  final clubLogo = File(p.join(
    scriptDir,
    '..',
    '..',
    '..',
  ...
  if (clubLogo.existsSync()) {
    clubLogo.copySync(p.join(staticAssets.path, 'club_logo.svg'));
  } else {
    stderr.writeln('Warning: club_logo.svg not found at ${clubLogo.path}');
  }

  // Walk every .html file and inject the link.
  var injected = 0;
  for (final f in output.listSync(recursive: true).whereType<File>()) {
    if (p.extension(f.path) != '.html') continue;
    if (_injectIntoHtml(f, output.path)) injected++;
  }

  stdout.writeln('\n✓ Themed $injected HTML files.');
  stdout.writeln('  Serve with: dart run bin/serve.dart themed');
}

String _detectBaseline(String root) {
  final work = Directory(p.join(root, 'work'));
  if (!work.existsSync()) return p.join(root, 'work', 'missing');
  for (final d in work.listSync().whereType<Directory>()) {
    final api = p.join(d.path, 'doc', 'api');
    if (File(p.join(api, 'index.html')).existsSync()) return api;
  }
  return p.join(root, 'work', 'missing');
}
...
bool _injectIntoHtml(File file, String outputRoot) {
  final raw = file.readAsStringSync();
  // Compute relative path back to static-assets/.
  final fromDir = p.dirname(file.path);
  String relTo(String name) {
    final t = p.join(outputRoot, 'static-assets', name);
    return p.relative(t, from: fromDir).replaceAll('\\', '/');
  }

  // Use a simple parse + serialize via package:html so we keep dartdoc's
  // existing structure intact. We *only* add nodes we own.
  final doc = parse(raw);
  final head = doc.head;
  ...
  final faviconRel = relTo('club_logo.svg');
  final existingIcon = head.querySelector('link[rel="icon"]');
  if (existingIcon != null) {
    existingIcon.attributes['href'] = faviconRel;
    existingIcon.attributes['type'] = 'image/svg+xml';
  } else {
    final iconEl = doc.createElement('link')
      ..attributes['rel'] = 'icon'
      ..attributes['type'] = 'image/svg+xml'
      ..attributes['href'] = faviconRel;
    head.append(iconEl);
  }

  final link = doc.createElement('link')
    ..attributes['rel'] = 'stylesheet'
    ..attributes['href'] = '${relTo('club-theme.css')}?v2'
    ..attributes['data-club-theme'] = 'css';
  head.append(link);

  final script = doc.createElement('script')
    ..attributes['src'] = '${relTo('club-theme.js')}?v2'
    ..attributes['defer'] = 'defer'
    ..attributes['data-club-theme'] = 'js';
  head.append(script);

  // package:html serialize() writes the modified doc back out.
  file.writeAsStringSync(doc.outerHtml);
  return true;
}

// ──────────────────────────────────────────────────────────────────
// club-theme.css — override dartdoc's --main-* palette with club's
// design tokens. Mirrors packages/club_web/src/app.css.
// ──────────────────────────────────────────────────────────────────
...
/* Neutralize dartdoc's `text-indent: -24px; margin-left: 24px` which
   ...
   Use inline-flex so the row keeps the dt's natural baseline but the
   chip sits next to the name with a real gap. */
section.summary dt,
div.summary dt,
.summary dt {
...
/* Pin footer to the bottom of short pages, otherwise dartdoc's
   white html background bleeds below. */
html, body {
  min-height: 100vh;
}
html {
...
/* ── Hide the dartdoc footer "https://dart.dev" advertising row,
   if present on some templates ─────────────────────────────── */
.no-break + a[href*="dart.dev"] { display: none; }
''';

// Runtime enhancements:
//   1. Better placeholder text on the search input
//   2. Copy button on every <pre> code block
//   3. Mark the active sidebar link (left + right) based on current URL,
//      and re-mark whenever dartdoc injects sidebar content async
...
(function () {
  ...
  // location.pathname is percent-encoded ("Type-safe%20routes…"),
  // but sidebar hrefs are raw attribute strings with literal spaces.
  // Decode both sides to a canonical form before comparing.
