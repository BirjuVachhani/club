...
Server-side site configuration uses top-level keys:

```yaml
# Optional: omit these to use the existing Club hostname.
# site_runner_url: https://preview.example.net
# site_runner_port: 8081
sites_path: /data/sites
max_sites: 20
max_site_archive_bytes: 104857600
max_sites_total_bytes: 524288000
max_site_expanded_bytes: 524288000
max_site_entries: 10000
```

Omit `site_runner_url` to use archived previews on the existing Club hostname. When set,
it requires `server_url` with a different hostname and a separate listener port.
See [runner deployment](/configuration/storage/#browser-previews) for these optional settings.
These server keys are distinct from the publisher's `sites` map in
[`club.yaml`](/guides/package-sites/#configure-your-sites).
...
# Permit packages to be marked public and read without credentials. Only
# the first of two gates: a server admin must also turn on the dashboard
# toggle, and each package must be marked public. Default: false
...
| `public_packages_enabled` | `PUBLIC_PACKAGES_ENABLED` |
...
| `site_runner_url` | `SITE_RUNNER_URL` |
| `site_runner_port` | `SITE_RUNNER_PORT` |
| `max_sites` | `MAX_SITES` |
| `max_site_archive_bytes` | `MAX_SITE_ARCHIVE_BYTES` |
| `max_sites_total_bytes` | `MAX_SITES_TOTAL_BYTES` |
| `max_site_expanded_bytes` | `MAX_SITE_EXPANDED_BYTES` |
| `max_site_entries` | `MAX_SITE_ENTRIES` |
