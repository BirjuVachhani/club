...
| `SITE_RUNNER_URL` | Unset | Fixed separate HTTPS origin for archived previews; unset means no runner listener |
| `SITE_RUNNER_PORT` | `8081` | Runner listener port, different from the main server port |
...
for deployment requirements and the fixed CLI/browser caps. URL-only targets do
not need `SITE_RUNNER_URL`. **Disable sites** is a runtime database setting in
**Admin > Sites**, not an environment variable; it defaults to off and does not
stop publishing. See [Package Sites](/guides/package-sites/).
