...
| `PUBLIC_PACKAGES_ENABLED` | No | `false` | Permit packages to be marked public and read without credentials |
...
Permits individual packages to be marked public and read without credentials. Default: `false`, so a club server is fully private until you opt in.

This is only the first of two gates. A server admin must also turn on the dashboard toggle under **Admin > Public packages**, and each package must be marked public on its own. Setting this to `false` again is a kill switch: every public package requires credentials on the next request, with no per-package state changed.

```bash
PUBLIC_PACKAGES_ENABLED=true
```

See the [Public Packages guide](/guides/public-packages/) for the full model.
...
`S3_BUCKET`, `S3_ACCESS_KEY`, and `S3_SECRET_KEY` are **required** when `BLOB_BACKEND=s3`. `S3_REGION` is optional and defaults to `us-east-1`. `S3_ENDPOINT` is optional for AWS but required for anything else.
...
| --- | --- | --- |
| `SITES_PATH` | `/data/sites` | Persistent local archives, URL destinations, and labels, independent of `BLOB_BACKEND` |
| `SITE_RUNNER_URL` | Unset | Optional separate HTTPS runner origin; unset uses same-host previews |
| `SITE_RUNNER_PORT` | `8081` | Optional separate-runner listener port, used only when SITE_RUNNER_URL is set |
| `MAX_SITES` | `20` | Targets per package, including URL targets |
| `MAX_SITE_ARCHIVE_BYTES` | `104857600` | Compressed bytes per site archive |
| `MAX_SITES_TOTAL_BYTES` | `524288000` | Total compressed site bytes per publish |
| `MAX_SITE_EXPANDED_BYTES` | `524288000` | Expanded bytes per archive, including tar headers and padding |
| `MAX_SITE_ENTRIES` | `10000` | Entries per site archive |

See [site storage and runner setup](/configuration/storage/#static-site-archives)
for deployment options and the fixed CLI/browser caps. Neither archived nor
URL-only targets require `SITE_RUNNER_URL`. **Enable sites** is a runtime database setting in
**Admin > Sites**, not an environment variable. It is unchecked by default,
so viewing sites requires administrator opt-in. Publishing is unchanged. See [Package Sites](/guides/package-sites/).
...
| `BLOB_BACKEND=s3` without `S3_BUCKET` | `S3 configuration required when BLOB_BACKEND=s3.` |
| `BLOB_BACKEND=s3` without access/secret keys | `S3_ACCESS_KEY and S3_SECRET_KEY are required when BLOB_BACKEND=s3.` |
