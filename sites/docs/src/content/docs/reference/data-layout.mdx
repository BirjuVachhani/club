...
│
├── sites/                          PRIMARY: back up
│   └── <package>/<site>/
│       ├── site.tar.gz              archived target
│       ├── redirect.json            URL target (instead of archive)
...
| Primary (sites) | `sites/` | **Yes, must** | **Yes** | No, loses uploaded previews and external links |
...
With local defaults, back up **three primary directories**: `db/`, `blobs/`, and
`sites/`. Site data stays local even when package blobs use S3 or GCS. Caches can
be regenerated; logs and temporary uploads are not primary state:
...
| Package sites | `SITES_PATH` | `sites_path` | `/data/sites` |
...
## Package site storage

`sites/` holds the active site set per package, not a history per version.
Archive targets store `site.tar.gz`; URL targets store `redirect.json` instead.
An optional `metadata.json` holds the label. None of these files are a cache.

Site replacement temporarily uses sibling `<package>.candidate` and
`<package>.backup` directories. Startup recovers interrupted replacements.
Use one server process and stop publishing during backup or restore so a site
replacement cannot race the copy. Include this directory even when package
blobs are backed up from object storage. See
[Package Sites](/guides/package-sites/) and
...
```bash
# Stop the server before copying primary state.
# Includes package sites; use your configured paths if different.
