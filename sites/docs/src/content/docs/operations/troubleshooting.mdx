...
3. **If using SQLite FTS5**, the index may be out of sync. The search-index layer rebuilds the `package_fts` table from the metadata store; restart the server to trigger a re-index.
