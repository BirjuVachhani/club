...
Returns `VersionScore`, which includes `grantedPoints`, `maxPoints`, `likeCount`, `downloadCount30Days`, and `tags`. For the full pana report (including pana/Dart/Flutter versions), use `getScoringReport(package, version)`, which returns a `PackageScoringReport`.
