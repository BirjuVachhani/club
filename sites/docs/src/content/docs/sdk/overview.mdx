...
<Tabs>
  ...
  club_api: ^0.3.0`} lang="yaml" title="pubspec.yaml" />
  </TabItem>
...
| `token` | Yes | API token (`club_pat_...`) or session cookie value. The parameter is required by the constructor. Pass an empty string if you only intend to call `login()`. |
...
| Area | Methods | Docs |
|------|---------|------|
| Packages | `listVersions`, `getVersion`, `downloadArchive`, `getOptions`, `setOptions`, `getVersionOptions`, `setVersionOptions`, `getScore`, `getScoringReport`, `getLikeCount`, `listAllNames`, `getVersionContent`, `getDartdocStatus`, `fetchBytes` | [Package Operations](/sdk/packages/) |
| Publishing | `publishFile`, `publish` | [Publishing](/sdk/publishing/) |
| Auth | `login`, `createApiKey`, `listApiKeys`, `revokeApiKey`, `getMe` | [Auth & Tokens](/sdk/auth/) |
| Account | `getMyPackages` | Packages owned by the current user |
...
- `VersionContent`: readme, changelog, example, and screenshots for a version
- `PackageDartdocStatus`: dartdoc generation status for a package
- `PackageScoringReport`: full pana scoring report for a version
