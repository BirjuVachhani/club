...
1. **Log in to your server**
   ...
   ```bash
   club login localhost:8080
   ```

   `club login` accepts a bare host. Local dev hosts (`localhost`, `127.0.0.1`, `0.0.0.0`) default to `http://`; everything else defaults to `https://`. A full URL works too.
