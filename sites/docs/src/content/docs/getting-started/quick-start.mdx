...
2. **Login to your server**
   ...
   `club login` accepts a bare host. `localhost`, `127.0.0.1`, and `0.0.0.0` default to `http://`; everything else defaults to `https://`. A full URL like `http://localhost:8080` is accepted too — both forms map to the same credential.
   ...
   <Aside>
     If you prefer a non-browser flow (for example, on a headless machine), use `club login localhost:8080 --no-browser` to fall back to a terminal email/password prompt, or pass an existing token with `--key club_pat_...`.
   </Aside>
