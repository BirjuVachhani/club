...
Only `--directory`, `--dry-run`, and `--force` behave as they do in
[`dart pub publish`](https://dart.dev/tools/pub/cmd/pub-publish); the
rest are club-specific. `club publish` also accepts flags not listed
here (`--enhanced`, `--auto`, `--from-git`, `--ref`). See
[`club publish`](/cli/publishing/) for the complete flag reference.
