...
A club server is private by default: every request must carry a valid session cookie or personal access token. The one exception is opt-in. If the operator has enabled [public packages](/guides/public-packages/) and marked a specific package public, that package can be read anonymously. Everything else, including every write, still requires authentication.
...
The browser UI authenticates with the session cookie; the CLI, API clients, and `dart pub` authenticate with a PAT. A session token presented as a `Bearer` header is also accepted, but a bearer-presented session does not slide its expiry, so it cannot keep itself alive without going back through the cookie flow. In practice, use a PAT for everything outside the browser.
...
<Tabs>
  ...
  ```bash
  club login myclub.birju.dev --key club_pat_...
  ```
  ...
  ```bash
  club login myclub.birju.dev --no-browser
  ```
