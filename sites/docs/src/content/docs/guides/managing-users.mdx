...
<Tabs>
  ...
  Response (`201 Created`) — the user fields are returned at the top level alongside `mode` and the generated password:
  ```json
  {
    "userId": "...",
    "email": "jane@example.com",
    ...
    "mode": "password",
    "generatedPassword": "7nQx2VpD9aLh6K3m"
  }
  ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  Response (`201 Created`) — the user fields are returned at the top level alongside `mode`, the invite URL, and its expiry window in hours:
  ```json
  {
    "userId": "...",
    "email": "jane@example.com",
