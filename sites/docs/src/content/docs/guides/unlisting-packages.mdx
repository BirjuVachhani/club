...
<Aside type="note">
Unlisting does not affect access control. It decides whether a package is *advertised*, not who may read it. Access is governed separately by the server's auth policy and, where the operator has enabled it, by each package's [visibility](/guides/public-packages/). The two compose: a package that is public and unlisted resolves for `dart pub get` without a token but appears in no listing.
</Aside>
...
Event kind: `package.options_updated` (with `isUnlisted` in the before/after payload).
