...
- Any named **uploader**, when the package is uploader-owned (no publisher set).
- Any **admin of the publisher** that owns it, when the package is publisher-owned.
...
Discontinuing, updating `replacedBy`, or un-discontinuing each append an entry to the package activity log recording the actor, a timestamp, and a summary:

```http
GET /api/packages/<pkg>/activity-log
```

Event kind: `package.options_updated`.
