...
Uploading to https://club.example.com...
...
1. **`--server <url>`** flag (highest priority). The URL must be in your
   logged-in credentials, otherwise the command exits with a hint to run
   `club login`.
...
3. **Auto-pick from logged-in servers**:
   ...
   - 0 servers: exits with a hint to run `club login <url>`.
...
club publish --server https://internal.club.example.com
...
- **All confirmation prompts can be skipped** with `--force`.
...
```yaml
- name: Publish
  run: club publish --force --server https://club.example.com
  env:
    CLUB_TOKEN: ${{ secrets.CLUB_TOKEN }}
```

`CLUB_TOKEN` shadows any stored credentials, so there is no need to pre-seed a credentials file. Alternatively, run `club login --key club_pat_... https://club.example.com` as a previous step to materialise the credentials file.
