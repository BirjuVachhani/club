---
title: club mcp
description: Run a stdio Model Context Protocol server so AI clients (Claude Code, Claude Desktop, Codex, Cursor, Gemini CLI, Cline, Continue, Zed) can browse, search, and reason about packages on a club server.
sidebar:
  order: 8
---
...
server over stdio. Once configured in your AI client (Claude Code,
Claude Desktop, Codex, Cursor, Gemini CLI, Cline, Continue, Zed, …),
your assistant can search packages on a
...
<Tabs>
  ...
  Register the server with `claude mcp add`. Everything after `--` is the
  command Claude Code spawns:

  ```bash
  claude mcp add club -- club mcp
  ```

  Flags go in the same place:

  ```bash
  claude mcp add club -- club mcp --server myclub.birju.dev
  ```
  ...
  |-------|------|----------------|
  | Local (default) | `--scope local` | Your own settings, this project only |
  | Project | `--scope project` | `.mcp.json` in the project root, shareable via git |
  | User | `--scope user` | Your own settings, every project |

  Project scope writes a plain `.mcp.json` you can also edit by hand:
  ...
  Run `/mcp` inside a Claude Code session to confirm `club` is connected
  and see its tools.

  <Aside type="caution" title="Don't commit tokens">
  `.mcp.json` is meant to be checked in. Keep `--token` out of it and let
  each teammate run `club login` instead.
  </Aside>
  </TabItem>
  <TabItem label="Codex">
  Codex CLI keeps MCP servers in `~/.codex/config.toml`. It uses TOML
  tables, and the key is `mcp_servers` (snake_case), not `mcpServers`:
  ...
  Pinning a server works the same way:
  ...
  To keep the token out of the args list, pass it through the
  environment instead:
  ...
  command = "club"
  args = ["mcp", "--server", "myclub.birju.dev"]
  env = { CLUB_TOKEN = "club_pat_..." }
  ```

  Or let Codex write the entry for you:

  ```bash
  codex mcp add club -- club mcp --server myclub.birju.dev
  ```

  Check it with `codex mcp list`, then start a new Codex session to pick
  up the change.
  </TabItem>
  <TabItem label="Gemini CLI">
  Gemini CLI reads `~/.gemini/settings.json` globally, or
  ...
  ```bash
  gemini mcp add club club mcp
  ```

  Run `gemini mcp list` to confirm registration, and `/mcp` inside a
  session to see the connected server and its tools.
  </TabItem>
...
[`club add`](/cli/overview/) when you want the CLI itself to write
the change.
</Aside>
