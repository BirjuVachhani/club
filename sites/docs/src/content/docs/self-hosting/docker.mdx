...
| Exposed port | 8080 |
| Data volume | `/data` |
| Runtime user | Server runs as `root` by default (override with `docker run --user` or compose `user:`). The pana scoring subprocess always drops to the unprivileged `scoring` user (UID/GID 1001). |
| Entrypoint | `/app/bin/entrypoint.sh` (prepares `/data` directories, then execs the AOT-compiled `/app/bin/server`) |
...
│           ├── blob
│           └── index.json
├── sites/               # Primary state: archives, URL destinations, labels
...
Back up all three primary tiers: `db/`, `blobs/`, and `sites/`. Site data stays local even with S3 or GCS blobs. Include your configured `SITES_PATH` if it is outside `/data`. The cache is re-derivable on next boot: for `dartdoc`, on the next scoring run per package; for SDKs/pub-cache, automatically. The `🟢` tiers hold only observability or in-flight data.
...
[Backups](#backing-up) section below.
...
By default the server runs as `root` inside the container, so it can write to a freshly created host directory without any `chown`:

```bash
sudo mkdir -p /var/lib/club
```

If you run the container as a non-root user (`docker run --user` or compose `user:`), make sure that UID owns the directory. The scoring subprocess always runs as UID/GID 1001 regardless; the entrypoint relaxes permissions on the shared cache/tmp paths when started as root.
...
The runtime image is minimal and does **not** include the `sqlite3` CLI. Run backups either via a short-lived sidecar container or by snapshotting the whole `/data` volume. Pause publishing for the entire database, blob, and site backup.
...
docker cp club:/data/sites ./sites-backup/

# Or stop the server before copying SQLite files directly.
# Snapshot all primary tiers; regenerable caches can be skipped.
...
docker run --rm -v club_data:/data -v "$PWD:/backup" \
  ...
  -C /data db blobs sites
docker compose start club
```

<Aside type="tip">
  For automated backups, use the [backup script](/operations/backup/#full-backup-script)
  with publishing paused for the entire backup. Direct SQLite file copies
  require the server to be stopped; do not schedule a live tar copy of `db/`.
</Aside>
...
# Restore active site archives, URL destinations, and labels
docker cp sites-backup/. club:/data/sites/
...
3. **`debian:bookworm-slim`** -- copies the binary + dynamic libraries and the web build, installs runtime dependencies (`ca-certificates`, `curl`, `git`, `unzip`, `webp`) needed by pana for scoring, and creates the unprivileged `scoring` user (UID/GID 1001) that the pana scoring subprocess drops to. The server itself runs as `root` by default.
