...
sudo mkdir -p /var/lib/club/db
sudo mkdir -p /var/lib/club/blobs
...
1. **Back up the database**
   ...
   ```bash
   sqlite3 /var/lib/club/db/club.db ".backup /var/lib/club/club-backup-$(date +%Y%m%d).db"
   ```
...
```bash
sqlite3 /var/lib/club/db/club.db ".backup /opt/backups/club-$(date +%Y%m%d).db"
```

### Package tarballs

```bash
tar czf /opt/backups/club-blobs-$(date +%Y%m%d).tar.gz -C /var/lib/club/blobs .
```
...
0 2 * * * club sqlite3 /var/lib/club/db/club.db ".backup /opt/backups/club-$(date +\%Y\%m\%d).db" && tar czf /opt/backups/club-blobs-$(date +\%Y\%m\%d).tar.gz -C /var/lib/club/blobs .
EOF
```
