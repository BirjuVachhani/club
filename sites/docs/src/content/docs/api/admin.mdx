...
The server-wide half of the [public packages](/guides/public-packages/) switch. Server admin only.
...
<Aside type="caution">
  Setting `enabled` back to false is a kill switch: every public package requires credentials again on the next request, with no per-package state changed. It does not recall anything already downloaded.
</Aside>
